 ...
 * @author Professor
 */
public class P04_MostrarAlumnesIProfessors {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            cad = cad + "where type(p) in :classes";
            
            Query q = em.createQuery(cad);
            q.setParameter("classes",Arrays.asList(Alumne.class,Professor.class) );
            
            List<Persona> ll = q.getResultList();
            if (ll.size() == 0) {
                System.out.println("No hi ha cap persona");
            } else {
                System.out.println("Persones a la BD:");
                for (Persona p : ll) {
