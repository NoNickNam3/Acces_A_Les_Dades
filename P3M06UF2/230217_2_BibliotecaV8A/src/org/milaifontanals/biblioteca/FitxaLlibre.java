...
public class FitxaLlibre extends Fitxa {
    ...
    @JoinTable(
            ...
            joinColumns = @JoinColumn(name = "llibre", referencedColumnName = "referencia",
                    foreignKey = @ForeignKey(name = "autoria_fk_f_llibre",
                            foreignKeyDefinition = "foreign key (llibre) references f_llibre on delete cascade")), // def. columna "llibre"
            inverseJoinColumns = @JoinColumn(name = "autor", referencedColumnName = "id",
                    foreignKey = @ForeignKey(name = "autoria_fk_autor",
                            foreignKeyDefinition = "foreign key(autor) references autor on delete cascade")), // def. columna "autor"
