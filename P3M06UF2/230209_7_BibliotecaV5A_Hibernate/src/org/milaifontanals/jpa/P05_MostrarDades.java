...
public class P05_MostrarDades {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            // Preparem consulta paramtetritzada per cercar llibres d'un autor
            // que passarem per paràmetre
            String sAux = "select fl from FitxaLlibre fl ";
            sAux = sAux + "join fl.autors a ";
            sAux = sAux + "where a = :autor ";    // autor és el paràmetre
            sAux = sAux + "order by fl.referencia";
            Query qAux = em.createQuery(sAux);

            System.out.println("Autors amb la seva autoria");
            System.out.println("**************************");
            consulta = "select a from Autor a order by a.nom";
            ...
            for (Autor a : autors) {
                ...
                // Cal executar la consulta que hem preparat prèviament
                qAux.setParameter("autor", a);
                // D'aquesta manera no ens hem de preocupar del tipus de dadad de getId()!!!
